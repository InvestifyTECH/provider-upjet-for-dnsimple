...
import (
	...
	apisNamespaced "github.com/InvestifyTECH/provider-upjet-for-dnsimple/apis/namespaced"
	"github.com/InvestifyTECH/provider-upjet-for-dnsimple/config"
	"github.com/InvestifyTECH/provider-upjet-for-dnsimple/internal/clients"
	controllerCluster "github.com/InvestifyTECH/provider-upjet-for-dnsimple/internal/controller/cluster"
	controllerNamespaced "github.com/InvestifyTECH/provider-upjet-for-dnsimple/internal/controller/namespaced"
	"github.com/InvestifyTECH/provider-upjet-for-dnsimple/internal/features"
	"github.com/InvestifyTECH/provider-upjet-for-dnsimple/internal/version"
)
...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for Upjet-for-DNSimple").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-upjet-for-dnsimple"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-upjet-for-dnsimple",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(mgr.GetScheme()), "Cannot add cluster-scoped Upjet-for-DNSimple APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(mgr.GetScheme()), "Cannot add namespaced Upjet-for-DNSimple APIs to scheme")
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated(mgr, clusterOpts), "Cannot setup cluster-scoped Upjet-for-DNSimple controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated(mgr, namespacedOpts), "Cannot setup namespaced Upjet-for-DNSimple controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup(mgr, clusterOpts), "Cannot setup cluster-scoped Upjet-for-DNSimple controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup(mgr, namespacedOpts), "Cannot setup namespaced Upjet-for-DNSimple controllers")
	}
